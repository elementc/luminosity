...
typedef struct {
  /* Geometry */
  GRect r_bounds; /* This may only be used by the analog layer, actually, in the
                     calculate_perimeter function, and therefore deletable if we
                     delete that feature. */
  GPoint p_center;
...
extern char date_buffer[HOURLY_WEATHER_DATA_BUFFER_LEN]; // todo: different,
                                                         // appropriate, len
extern char time_buffer[6];
extern char temperature_buffer[8];
extern char forecast_high_low_buffer
    [HOURLY_WEATHER_DATA_BUFFER_LEN]; // todo: different, appropriate, len
extern char
    wind_speed_buffer[HOURLY_WEATHER_DATA_BUFFER_LEN]; // todo: different,
                                                       // appropriate, len
extern char
    wind_bearing_buffer[HOURLY_WEATHER_DATA_BUFFER_LEN]; // todo: different,
                                                         // appropriate, len
