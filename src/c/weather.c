...
void bearing_icon_update_proc(Layer* layer, GContext* ctx){
  if (s_weather_ready){
    ...
    GPoint center = GPoint(bounds.origin.x + radius + 3, bounds.origin.y + radius + 3);
    graphics_context_set_stroke_width(ctx, 3);
    graphics_context_set_stroke_color(ctx, COLOR_TIME);
    graphics_draw_circle(ctx, center, radius);
    GPoint edge = gpoint_from_polar(bounds, GOvalScaleModeFillCircle, DEG_TO_TRIGANGLE(windBearing));
